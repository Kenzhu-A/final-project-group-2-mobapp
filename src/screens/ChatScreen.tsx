...
export default function ChatScreen({ route, navigation }: any) {
  ...
  useEffect(() => {
    ...
    socketRef.current.on("receive_message", (newMessage) => {
      setMessages((prev) => {
        ...
        // 2. FIX: Prevent Ghost Messages. Replace the temporary optimistic message with the real one.
        const ghostIndex = prev.findIndex(
          (msg) =>
            msg.isOptimistic &&
        ...
        if (ghostIndex !== -1) {
          ...
          newMessages[ghostIndex] = newMessage;
          return newMessages;
        }

        // If no ghost exists, just append the message normally
  ...
  const sendMessage = async () => {
    ...
    if (editingMessageId) {
      ...
      setMessages((prev) =>
        prev.map((m) => (m.id === originalId ? { ...m, text: textToSend } : m)),
      );
      try {
  ...
  const renderMessage = ({ item }: { item: any }) => {
    ...
    return (
      ...
      >
        ...
        {/* FIX: Add Sent Label only for actual completed messages sent by you */}
...
const styles = StyleSheet.create({
  ...
  // FIX: Message Wrapper & Sent Label styling
  messageWrapper: { maxWidth: "80%", marginBottom: 10 },
  messageBubble: {
    paddingHorizontal: 16,
    paddingVertical: 12,
    borderRadius: 20,
