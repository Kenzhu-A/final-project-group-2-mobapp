    ...
    useCallback(() => {
      const fetchLikedPosts = async () => {
        try {
          ...
          // 2. Get the user's specific liked memory array
          const stored = await AsyncStorage.getItem(`${userId}_likedPosts`);
          const likedIds = stored ? JSON.parse(stored) : [];
          
          // 3. Filter down
          const filtered = allPosts.filter((post: any) => likedIds.includes(post.id));
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.background }]}>
      ...
      {likedPosts.length === 0 ? (
        <View style={styles.emptyContainer}>
          ...
          <Text style={[styles.emptyText, { color: colors.textPrimary }]}>No liked posts yet.</Text>
        </View>
      ) : (
        <FlatList
          data={likedPosts}
