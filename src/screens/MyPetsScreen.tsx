...
export default function MyPetsScreen({ navigation }: any) {
  ...
  const toggleStatus = async (pet: any) => {
    ...
    setMyPets((prev) =>
      prev.map((p) => (p.id === pet.id ? { ...p, status: newStatus } : p)),
    );
    ...
    try {
      ...
      // Revert if failed
      setMyPets((prev) =>
        prev.map((p) => (p.id === pet.id ? { ...p, status: pet.status } : p)),
      );
    }
  };
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              ...
              {/* Added flex: 1 here so it fills the middle space without pushing the button out */}
              <View style={styles.petDetails}>
                ...
                {/* Display the new Category & Breed */}
                <Text
                  style={[styles.petInfo, { color: colors.textSecondary }]}
                  numberOfLines={1}
                >
...
const styles = StyleSheet.create({
  ...
  petCard: { 
    flexDirection: 'row', 
    alignItems: 'center', 
    justifyContent: 'space-between', // Ensures contents space out perfectly
    padding: 12, 
    borderRadius: 16, 
    marginBottom: 16, 
    borderWidth: 1 
  ...
  petDetails: { 
    flex: 1, // CRITICAL: Makes the text area shrink/grow dynamically
    marginRight: 8,
    justifyContent: 'center'
  },
  ...
  toggleBtn: { 
    ...
    borderRadius: 8, 
    borderWidth: 1,
    minWidth: 100, // Keeps the button a consistent size
    alignItems: 'center'
  },
