...
export default function PetChatsScreen({ navigation }: any) {
  ...
  useFocusEffect(
    useCallback(() => {
      setLoading(true);
      Promise.all([AsyncStorage.getItem('userId'), AsyncStorage.getItem(CHAT_READ_KEY)]).then(([id, rawReadMap]) => {
        setCurrentUserId(id);
  ...
  const withUnreadMeta = useMemo(
    () =>
      conversations.map((item) => {
        ...
        const isUnread = !readTs || new Date(latestTs).getTime() > new Date(readTs).getTime();
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          contentContainerStyle={{ paddingBottom: 110, paddingHorizontal: 16, paddingTop: 10 }}
          showsVerticalScrollIndicator={false}
          ListEmptyComponent={
            <View style={{ alignItems: 'center', marginTop: 40 }}>
              <Ionicons name="chatbubbles-outline" size={60} color={colors.border} />
              <Text style={{ color: colors.textSecondary, marginTop: 12, fontFamily: 'DMSans_400Regular' }}>
...
const styles = StyleSheet.create({
  ...
  searchWrap: { flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderRadius: 12, paddingHorizontal: 12, marginHorizontal: 16, marginTop: 12, height: 42 },
  searchInput: { flex: 1, marginLeft: 8, fontSize: 14, fontFamily: 'DMSans_400Regular' },
  tabRow: { flexDirection: 'row', gap: 8, paddingHorizontal: 16, marginTop: 12 },
  tabPill: { borderWidth: 1, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 7 },
